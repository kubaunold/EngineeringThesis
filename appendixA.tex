\chapter{CD/DVD included}
% \section{CD/DVD included}
Attached CD/DVD includes the following directories:

\dirtree{%
.1 /.
.2 MARS\_App.zip \ldots{}
\begin{minipage}[t]{8cm}
    This is a zipped file containing entire projects that create the solution as well as \textit{{.}sln} file for easy further development in Microsoft Visual Studio 2019{.}
\end{minipage}.
.2 thesis\_pdf \ldots{} 
\begin{minipage}[t]{8cm}
    This directory contains \textit{{.}pdf} version of the Engineering Thesis{.}
\end{minipage}.
}

\vspace{1cm}
\noindent
Alternatively, application and this thesis can be found under these links: \\
\url{https://github.com/kubaunold/MARS_App.git} \\
\url{https://github.com/kubaunold/EngineeringThesis.git}
